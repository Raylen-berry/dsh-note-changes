...
> **v1.6.2 已真机端到端验证通过**（2026-09-20，第三次重启后）：回执尾 `，已提交并推送`，插件自己产出的提交
> 立刻出现在远端与右侧抽屉里，且用户另一份未提交的改动没有被扫进提交。
> 三轮才修完（接线 → write 腿缺策略 → network 腿禁建管道），每一层都只在真实写入时暴露——离线套件 55 项全绿
> 与 CI success 都没能提前抓到它们。
